...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        //Initialize函数：输入当前帧mCurrentFrame；两帧的特征点匹配关系mvIniMatches
        //输出：初始化的位姿关系Rcw, tcw；得到的3D点云mvIniP3D，以及点云是否可三角化vbTriangulated
        if(mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            //每一个MapPoint的生成需要更新：1）所有关键帧对该MapPoint的Observation
            //                                                 2）挑选区分度最高的描述子（中值代表整体）
            //                                                 3）所有关键帧到该MapPoint的观测方向的平均以及观测距离的上下线dmin，dmax
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        // 步骤4.3：表示该KeyFrame的哪个特征点可以观测到哪个3D点（对应关键帧而言，特征点和MapPoint的对应）
        pKFini->AddMapPoint(pMP,i);
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);

        // a.表示该MapPoint可以被哪个KeyFrame的哪个特征点观测到（对于MapPoint而言，特征点和MapPoint的对应）
        pMP->AddObservation(pKFini,i);
        pMP->AddObservation(pKFcur,mvIniMatches[i]);

        // b.从众多观测到该MapPoint的特征点中挑选区分读最高的描述子（以描述子的中值来代表整体）
        pMP->ComputeDistinctiveDescriptors();
        // c.更新该MapPoint平均观测方向以及观测距离的范围
        //平均观测方向：所有观测到该MapPoint的关键帧与该MapPoint连成的向量的和
        //观测距离：观测到该点的距离上下线（和金字塔层数和尺度因子有关）
...
//TODO：当前帧的parent帧是与当前帧共视程度最高的关键帧
...
bool Tracking::TrackLocalMap()
{
    ...
    //mvpLocalKeyFrames是所有和当前帧共视的关键帧
    //mvpLocalMapPoints是所有mvpLocalKeyFrames上能看到的MapPoint
    //TODO:在tracking线程中，只关联图像帧和MapPoint的关系，但不产生新的MapPoint
    //而在LocalMap中，会产生新的MapPoint
    UpdateLocalMap();
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        // 步骤2.1：判断LocalMapPoints中的点是否在在视野内，若在视野范围内，则准备将其投影到当前帧
    ...
    if(nToMatch>0)
    {
        ...
        // 步骤2.2：对视野范围内的MapPoints通过投影进行特征点匹配，若匹配上，则为mCurrentFrame增加MapPoint
...
void Tracking::UpdateLocalMap()
{
    ...
    // Update
    // 更新局部关键帧和局部MapPoints
    //三个策略：1：能观测到当前帧MapPoints的关键帧作为局部关键帧
    //         2：与策略1得到的局部关键帧共视程度很高的关键帧作为局部关键帧（二级共视）
    //         3：当前帧的子关键帧
    //         4：当前帧的父关键帧
    //=================以上就是局部地图中局部关键帧的定义===========================
    UpdateLocalKeyFrames();
    //所有LocalMap关键帧能看到的MapPoint都是LocalMap的LocalMapPoint
    UpdateLocalPoints();
}
