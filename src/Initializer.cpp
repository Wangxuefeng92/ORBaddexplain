                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    // 计算fundamental matrix并打分，并得到内点vbMatchesInliersF，得分SF以及F矩阵
                            ...
                            cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
    ...
    // Compute Essential Matrix from Fundamental Matrix基础矩阵E21
    ...
    //输入：变换矩阵R1，t1；两帧特征点mvKeys1，mvKeys2，两帧间特征点的匹配情况mvMatches12；匹配的内点vbMatchesInliers，内参K
    //输出：是否三角化vbTriangulated1，视差角parallax1
    ...
    if(maxGood==nGood1)
    {
        if(parallax1>minParallax)//初始化要求有较大的视差角
        {
            vP3D = vP3D1;
                       ...
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        cv::Mat p3dC2 = R*p3dC1+t;//TODO：函数中的p3dC1和p3dC2都是在相机坐标系的3d点的坐标
